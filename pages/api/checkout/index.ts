...
export default async function handler(
  ...
  res: NextApiResponse
   ...
   if (req.method == 'POST')
   {
    ...
    if(result == 'Error account number not found.' || result == 'Error, insufficient balance for purchase')
    {
      res.status(401).end();
      return
    }
    result == null ? res.status(401).end() : res.status(200).json({bankaccountnumber: bk.number, balance: bk.balance})
...
async function postCheckout(
  ...
  try 
  {
    var account = await prisma.bankAccount.findFirst({where:{number: bkacct}})
    if (account.balance>=pric){
        var account_paid= await prisma.bankAccount.update({where:{number:bkacct},data:{balance:account.balance-pric}})
        return account_paid}
