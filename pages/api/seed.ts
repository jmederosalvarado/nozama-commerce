...
export default async function populate(){
    ...
    for (let i =0 ; i<100; i++)
    {
        ...
        try
        {
            await prisma.user.create({data:{username:user1,password:pass1,image:ima1,rating:rat1}})
            await prisma.user.create({data:{username:user2,password:pass2,image:ima2,rating:rat2}})
            await prisma.bankAccount.create({data:{number:parseInt(String(bank1)),balance:parseFloat(String(bala1))}})
            await prisma.bankAccount.create({data:{number:parseInt(String(bank2)),balance:parseFloat(String(bala2))}})


            if (i%2 == 0)
            {
                await prisma.offer.create({data:{product:prod,productDescription:prodDescrition,idSeller:user1,price:currPr,rating:rat3,image:ima3}})
            }
            else
            {
                await prisma.auction.create({data:{product:prod,productDescription:prodDescrition,idSeller:user1,currentPrice:currPr,idBidder:user2,bids:bids,closing:almostDate,rating:rat3,image:ima3}})
            }
