...
export default async function handler(
  ...
  res: NextApiResponse<AuctionPreview | AuctionPreview[]>
    ...
    else if (req.method == 'POST')
    {
      ...
      result ==null ? res.status(401).end() : res.status(200).json({id:String(offer.id), name: offer.product,image: offer.image,price:offer.currentPrice,bids:offer.bids})
    }
...
async function createAuction(req:NextApiRequest) {
    ...
    const { prod, prodDescription, idSeller,image,price,rating,closing } = req.body;   
    ...
    try 
    {
      const result = await prisma.auction.create({data:{product:pr,productDescription:prD,idSeller:idS,image:im,currentPrice: pric,rating:rate,closing:closin}})
      return result
