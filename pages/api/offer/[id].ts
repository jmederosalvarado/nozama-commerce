...
export default async function handler(
  ...
  res: NextApiResponse<OfferDetails>
    ...
    if (req.method == 'GET')
    {
      ...
      result == null ? res.status(401).end() :res.status(200).json({id: String(result.id)  ,name:result.product, description: result.productDescription, price:result.price, rating : result.rating  })
    }
    else if (req.method == 'DELETE')
    {
      ...
      result == null ? res.status(401).end() : res.status(200).json({id: String(offer.id),name: offer.product,description: offer.productDescription, price: offer.price, rating:offer.rating})
    }
    else if (req.method == "PUT")
    {
      const offer = await updateOffer(req)
      offer == null ? res.status(401).end() : res.status(200).json({id: String(offer.id),name: offer.product,description: offer.productDescription, price: offer.price, rating:offer.rating})
    }
}
...
async function updateOffer(
  req: NextApiRequest
    ...
    const result = await prisma.offer.update({where:{id :ID}, data:{product:prod,productDescription:prodD,price:pric}})
    return result
