...
export default async function handler(
  ...
  res: NextApiResponse<OfferPreview | OfferPreview[]>
    ...
    else if (req.method == 'POST')
    {
      ...
      result ==null ? res.status(401).end() : res.status(200).json({id:String(offer.id), name: offer.product,image: offer.image,price:offer.price, rating:offer.rating})
    }
...
async function createOffer(req:NextApiRequest) {
    ...
    try 
    {
      const result = await prisma.offer.create({data:{product:pr,productDescription:prD,idSeller:idS,image:im,price: pric,rating:rate}})
      return result
